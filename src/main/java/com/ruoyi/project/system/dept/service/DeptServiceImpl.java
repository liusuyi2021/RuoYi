...
public class DeptServiceImpl implements IDeptService
{
    ...
    @Resource
    private TrafficDeviceInfoMapper deviceInfoMapper;
    /**
     * 查询部门管理数据
     * 
     * @param dept 部门信息
    ...
    @Override
    @DataScope(deptAlias = "d")
    public List<Ztree> selectDeptTreeView(Dept dept)
    {
        List<Dept> deptList = deptMapper.selectDeptList(dept);
        List<Dept> deptListDev = deviceInfoMapper.selectTrafficDeviceInfoToDept();
        //接下来集合合并
        deptList.addAll(deptListDev);
        List<Ztree> ztrees = initZtree(deptList);
        return ztrees;
    }
    /**
    ...
    public List<Ztree> initZtree(List<Dept> deptList, List<String> roleDeptList)
    {
        ...
        for (Dept dept : deptList)
        {
            if (UserConstants.DEPT_NORMAL.equals(dept.getStatus()))
            {
                ...
                ztree.setChanNum(dept.getChanNum());
                ztree.setDeviceName(dept.getDeviceName());
                ztree.setIpAddr(dept.getIpAddr());
                ztree.setPort(dept.getPort());
                ztree.setUserName(dept.getUserName());
                ztree.setPassWord(dept.getPassWord());
                ztree.setDeviceType(dept.getDeviceType());
                if(dept.getIpAddr()!=null)
                {
                    ztree.setIconSkin("stopLive");
                }
                else
                {
                    ztree.setIconSkin("controlunit");
                }
                if (isCheck)
                {
